...
public class ProjectBuilderNode {
    ...
    public static AsyncNodeAction<MessagesState<String>> create() {
        return node_async(state -> {
            ...
            try {
                VueProjectBuilderTool vueBuilder = SpringContextUtil.getBean(VueProjectBuilderTool.class);
                // 执行 Vue 项目构建（npm install + npm run build）
                boolean buildSuccess = vueBuilder.buildProject(generatedCodeDir, VueProjectBuildToolEnum.PNPM_MODE);
                if (buildSuccess) {
                    // 构建成功，返回 dist 目录路径
